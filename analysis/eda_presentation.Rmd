...
\begin{itemize}
  ...
  \item{Calculate total (part + full) experience}
  \item{Calculate part time share}
  \item{Dummy encoding (advdeg,colldeg)}
  \item{Target encoding (profession,race,sex, area, msa)}
...
preprocess_recipe = recipe(lnwage ~ ., train_set_transformed) %>% 
  update_role(ID,new_role = "id variable") %>% # 
  step_rm(ends_with("sq"))%>% # Drop squared features
  step_YeoJohnson(all_numeric(),-ID,-lnwage) %>% # Transform numeric outliers
  step_mutate(total_exp = expp + expf + 0.01) %>% # Sum all experience
  step_mutate(exp_sq = total_exp ^ 2) %>% # Calculate squared experience
  step_mutate(part_time_share = expp / (total_exp)) %>% # Calculate part time share
  step_rm(c(expp,expf)) %>% # Drop unnecessary features
  step_dummy(c(advdeg,colldeg)) %>% # Dummy encode 
  step_lencode_glm(c(profession,race,sex, area, msa),outcome = vars(lnwage)) %>% # Target encode
  step_interact(~sex:race) %>% # Interaction between sex and race
  step_interact(~edyrs:race) %>% # Interaction between education and race
  step_normalize(all_numeric(), -all_outcomes()) # Scale and center all numeric features
...
knitr::include_graphics("C://Users//Home//Desktop//lasso_selection.png")
